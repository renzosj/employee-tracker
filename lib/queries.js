...
const viewRoles = () =>
 db.query('SELECT role.id AS Role_id, title AS Job_Title, department.name AS Department, salary AS Salary FROM role AS Role JOIN department ON role.department_id = department.id', (err, results) => {
    renderTable(results);
});

const viewEmployees = () =>
 db.query('SELECT employee.id AS Employee_id, first_name AS First_Name, last_name AS Last_Name, manager_id AS Manager_id, role.title AS Job_Title, salary AS Salary, department.name AS Department FROM employee JOIN role ON employee.role_id = role.id JOIN department ON role.department_id = department.id', (err, results) => {
    renderTable(results);
});
